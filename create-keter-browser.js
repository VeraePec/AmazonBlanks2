// Browser script to create the Keter Storage Shed product
// Run this in your browser console on the admin dashboard

async function createKeterProductInBrowser() {
  try {
    console.log('🚀 Creating Keter Storage Shed product...');
    
    // Import the utility function
    const { createKeterStorageShed } = await import('./src/utils/createProductFromInfo.ts');
    ...
    if (result.success) {
      ...
      const { saveAdCopy } = await import('./src/utils/generateAdCopy.ts');
...
window.createKeterProduct = createKeterProductInBrowser;

console.log('✅ Keter product creation script loaded!');
console.log('Run createKeterProduct() in the console to create the product.');
